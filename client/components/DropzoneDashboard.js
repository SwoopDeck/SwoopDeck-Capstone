import React from 'react';

function DropzoneDashboard() {
  return (
    <div className="flex-right">
      <div className="dropzone-dashboard screen">
        <div className="dz-title">
          <div className="dz-title-div">
            <div className="dz-title-div2">
              <div className="hey-dropzone-name">
                <span className="manrope-bold-shark-18px">
                  Dropzone Dashboard
                </span>
                <span className="span3">
                  {' '}
                  - here’s what’s happening this month
                </span>
              </div>
            </div>
          </div>
        </div>
        <div className="dz-body">
          <div className="flex-row-10">
            <div className="overlap-group-container">
              <div className="overlap-group10">
                <div className="text plusjakartasans-bold-eerie-black-16px">
                  Sales Report
                </div>
                <div className="overlap-20">
                  <div className="sales-report-lines">
                    <img
                      className="line-95"
                      src="https://anima-uploads.s3.amazonaws.com/projects/630e6c3ef11c17b54f51d1b7/releases/6316430df11c17b54f51f346/img/line-91@1x.svg"
                    />
                    <img
                      className="line-9"
                      src="https://anima-uploads.s3.amazonaws.com/projects/630e6c3ef11c17b54f51d1b7/releases/6316430df11c17b54f51f346/img/line-91@1x.svg"
                    />
                    <img
                      className="line-9"
                      src="https://anima-uploads.s3.amazonaws.com/projects/630e6c3ef11c17b54f51d1b7/releases/6316430df11c17b54f51f346/img/line-91@1x.svg"
                    />
                    <img
                      className="line-9"
                      src="https://anima-uploads.s3.amazonaws.com/projects/630e6c3ef11c17b54f51d1b7/releases/6316430df11c17b54f51f346/img/line-91@1x.svg"
                    />
                    <img
                      className="line-9"
                      src="https://anima-uploads.s3.amazonaws.com/projects/630e6c3ef11c17b54f51d1b7/releases/6316430df11c17b54f51f346/img/line-91@1x.svg"
                    />
                  </div>
                  <div className="graph1">
                    <img
                      className="bg1"
                      src="https://anima-uploads.s3.amazonaws.com/projects/630e6c3ef11c17b54f51d1b7/releases/6316430df11c17b54f51f346/img/bg@1x.svg"
                    />
                    <div className="flex-row-20">
                      <div className="feb plusjakartasans-medium-trout-13px2">
                        Feb
                      </div>
                      <div className="place plusjakartasans-medium-trout-13px2">
                        Mar
                      </div>
                      <div className="apr plusjakartasans-medium-trout-13px2">
                        Apr
                      </div>
                      <div className="flex-row-item plusjakartasans-medium-trout-13px2">
                        May
                      </div>
                      <div className="flex-row-item-10 plusjakartasans-medium-trout-13px2">
                        Jun
                      </div>
                      <div className="jul plusjakartasans-medium-trout-13px2">
                        Jul
                      </div>
                      <div className="flex-row-item-20 plusjakartasans-medium-trout-13px2">
                        Aug
                      </div>
                      <div className="flex-row-item-20 plusjakartasans-medium-trout-13px2">
                        Sep
                      </div>
                      <div className="flex-row-item-20 plusjakartasans-medium-trout-13px2">
                        Oct
                      </div>
                      <div className="flex-row-item-20 plusjakartasans-medium-trout-13px2">
                        Nov
                      </div>
                      <div className="flex-row-item plusjakartasans-medium-trout-13px2">
                        Dec
                      </div>
                      <div className="flex-row-item-15 plusjakartasans-medium-trout-13px2">
                        Jan
                      </div>
                    </div>
                  </div>
                  <img
                    className="sales-report-y-axis-line"
                    src="https://anima-uploads.s3.amazonaws.com/projects/630e6c3ef11c17b54f51d1b7/releases/6316430df11c17b54f51f346/img/sales-report-y-axis-line@2x.svg"
                  />
                  <div className="ellipse-21"></div>
                  <div className="overlap-group-1">
                    <div className="price-1 plusjakartasans-bold-eerie-black-13px">
                      $45,591
                    </div>
                    <div className="name">June 2021</div>
                  </div>
                </div>
              </div>
              <div className="transaction-title-container">
                <div className="transaction-title-2-1">
                  <div className="overlap-group-3">
                    <div className="ellipse-8-1"></div>
                    <div className="pending">Pending</div>
                  </div>
                  <div className="z2-1">
                    <div className="account-882 plusjakartasans-bold-eerie-black-13px">
                      Account&nbsp;&nbsp;****882
                    </div>
                    <div className="bank-payment plusjakartasans-medium-sonic-silver-13px">
                      Bank payment
                    </div>
                  </div>
                  <div className="x3-1">
                    <div className="price-3 plusjakartasans-bold-eerie-black-13px">
                      $249.94
                    </div>
                    <div className="date-1 plusjakartasans-medium-sonic-silver-13px">
                      Jan 17, 2022
                    </div>
                  </div>
                  <div className="netflix plusjakartasans-medium-sonic-silver-13px">
                    Netflix
                  </div>
                  <img
                    className="icon-outlinedots-horizontal-1"
                    src="https://anima-uploads.s3.amazonaws.com/projects/630e6c3ef11c17b54f51d1b7/releases/6316430df11c17b54f51f346/img/icon-outline-dots-horizontal@2x.svg"
                  />
                </div>
                {/* <div className="transaction-title-2">
                  <div className="overlap-group-2">
                    <div className="ellipse-8"></div>
                    <div className="completed plusjakartasans-medium-parsley-12px">
                      Completed
                    </div>
                  </div>
                  <div className="z2">
                    <p className="visa-card-4831 plusjakartasans-bold-eerie-black-13px">
                      Visa card&nbsp;&nbsp;**** 4831
                    </p>
                    <div className="card-payment plusjakartasans-medium-sonic-silver-13px">
                      Card payment
                    </div>
                  <div className="x3">
                    <div className="price-2 plusjakartasans-bold-eerie-black-13px">
                      $182.94
                    </div>
                    <div className="date plusjakartasans-medium-sonic-silver-13px">
                      Jan 17, 2022
                    </div>
                  </div>
                  <div className="amazon plusjakartasans-medium-sonic-silver-13px">
                    Amazon
                  </div>
                  <img
                    className="icon-outlinedots-horizontal"
                    src="https://anima-uploads.s3.amazonaws.com/projects/630e6c3ef11c17b54f51d1b7/releases/6316430df11c17b54f51f346/img/icon-outline-dots-horizontal@2x.svg"
                  />
                  </div>
                </div> */}
                <div className="transaction-title-2-1">
                  <div className="overlap-group-3">
                    <div className="ellipse-8-1"></div>
                    <div className="pending">Pending</div>
                  </div>
                  <div className="z2-1">
                    <div className="account-882 plusjakartasans-bold-eerie-black-13px">
                      Account&nbsp;&nbsp;****882
                    </div>
                    <div className="bank-payment plusjakartasans-medium-sonic-silver-13px">
                      Bank payment
                    </div>
                  </div>
                  <div className="x3-1">
                    <div className="price-3 plusjakartasans-bold-eerie-black-13px">
                      $249.94
                    </div>
                    <div className="date-1 plusjakartasans-medium-sonic-silver-13px">
                      Jan 17, 2022
                    </div>
                  </div>
                  <div className="netflix plusjakartasans-medium-sonic-silver-13px">
                    Tandem
                  </div>
                  <img
                    className="icon-outlinedots-horizontal-1"
                    src="https://anima-uploads.s3.amazonaws.com/projects/630e6c3ef11c17b54f51d1b7/releases/6316430df11c17b54f51f346/img/icon-outline-dots-horizontal@2x.svg"
                  />
                </div>
                <div className="transaction-title-2-2">
                  <div className="overlap-group-4">
                    <div className="ellipse-8-2"></div>
                    <div className="canceled">Canceled</div>
                  </div>
                  <div className="x2-2">
                    <p className="amex-card-5666 plusjakartasans-bold-eerie-black-13px">
                      Amex card&nbsp;&nbsp;**** 5666
                    </p>
                    <div className="card-payment-1 plusjakartasans-medium-sonic-silver-13px">
                      Card payment
                    </div>
                  </div>
                  <div className="x3-2">
                    <div className="price-4 plusjakartasans-bold-eerie-black-13px">
                      $199.24
                    </div>
                    <div className="date-2 plusjakartasans-medium-sonic-silver-13px">
                      Jan 17, 2022
                    </div>
                  </div>
                  <div className="amazon-prime plusjakartasans-medium-sonic-silver-13px">
                    Tandem
                  </div>
                  <img
                    className="icon-outlinedots-horizontal-2"
                    src="https://anima-uploads.s3.amazonaws.com/projects/630e6c3ef11c17b54f51d1b7/releases/6316430df11c17b54f51f346/img/icon-outline-dots-horizontal@2x.svg"
                  />
                </div>
                <div className="transaction-title-2-3">
                  <div className="overlap-group-5">
                    <div className="ellipse-8"></div>
                    <div className="completed plusjakartasans-medium-parsley-12px">
                      Completed
                    </div>
                  </div>
                  <div className="x02">
                    <div className="mastercard-6442 plusjakartasans-bold-eerie-black-13px">
                      Mastercard&nbsp;&nbsp;**** 6442
                    </div>
                    <div className="card-payment-2 plusjakartasans-medium-sonic-silver-13px">
                      Card payment
                    </div>
                  </div>
                  <div className="x03">
                    <div className="price-5 plusjakartasans-bold-eerie-black-13px">
                      $99.00
                    </div>
                    <div className="date-3 plusjakartasans-medium-sonic-silver-13px">
                      Jan 17, 2022
                    </div>
                  </div>
                  <div className="facebook plusjakartasans-medium-sonic-silver-13px">
                    Rental
                  </div>
                  <img
                    className="icon-outlinedots-horizontal-3"
                    src="https://anima-uploads.s3.amazonaws.com/projects/630e6c3ef11c17b54f51d1b7/releases/6316430df11c17b54f51f346/img/icon-outline-dots-horizontal@2x.svg"
                  />
                </div>
                <div className="transaction-title">
                  <div className="flex-col">
                    <div className="transactions plusjakartasans-bold-eerie-black-16px">
                      Transactions
                    </div>
                    <p className="lorem-ipsum-dolor-si jakartasans-normal-sonic-silver-13px">
                      Lorem ipsum dolor sit amet, consectetur adipis.
                    </p>
                  </div>
                  <div className="button-primary-with-small-icon-right">
                    <div className="frame-10">
                      <div className="button-name valign-text-middle">
                        See All Transactions
                      </div>
                      <div className="icon">
                        <img
                          className="icon-outlinecheveron-right"
                          src="https://anima-uploads.s3.amazonaws.com/projects/630e6c3ef11c17b54f51d1b7/releases/6316430df11c17b54f51f346/img/icon-outline-cheveron-right@2x.svg"
                        />
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div className="overlap-group-container-1">
              <div className="overlap-group4">
                <div className="flex-row-3">
                  <div className="text-1 plusjakartasans-bold-eerie-black-16px">
                    Traffic Sources
                  </div>
                  <div className="sort">
                    <div className="last-7-days">Last 7 Days</div>
                    <div className="icon-1"></div>
                  </div>
                </div>
                <div className="instagram">
                  <div className="flex-row">
                    <div className="title plusjakartasans-medium-eerie-black-13px">
                      Direct
                    </div>
                    <div className="digits plusjakartasans-medium-eerie-black-13px">
                      1,43,382
                    </div>
                  </div>
                  {/* <div className="overlap-group">
                    <img
                      className="elements-line-gorizontal"
                      src="https://anima-uploads.s3.amazonaws.com/projects/630e6c3ef11c17b54f51d1b7/releases/6316430df11c17b54f51f346/img/elements---line---gorizontal-7@2x.svg"
                    />
                  </div> */}
                </div>
                <div className="overlap-group4-item">
                  <div className="flex-row">
                    <div className="title-111 plusjakartasans-medium-eerie-black-13px">
                      Referral
                    </div>
                    <div className="digits-1 plusjakartasans-medium-eerie-black-13px">
                      87,974
                    </div>
                  </div>
                  <div className="overlap-group2-1">
                    {/* <img
                      className="elements-line-gorizontal-1"
                      src="https://anima-uploads.s3.amazonaws.com/projects/630e6c3ef11c17b54f51d1b7/releases/6316430df11c17b54f51f346/img/elements---line---gorizontal-5@2x.svg"
                    /> */}
                  </div>
                </div>
                <div className="overlap-group4-item">
                  <div className="flex-row">
                    <div className="title-2 plusjakartasans-medium-eerie-black-13px">
                      Social Media
                    </div>
                    <div className="digits-2 plusjakartasans-medium-eerie-black-13px">
                      45,211
                    </div>
                  </div>
                  {/* <div className="overlap-group">
                    <img
                      className="elements-line-gorizontal-2"
                      src="https://anima-uploads.s3.amazonaws.com/projects/630e6c3ef11c17b54f51d1b7/releases/6316430df11c17b54f51f346/img/elements---line---gorizontal-3@2x.svg"
                    />
                  </div> */}
                </div>
                <div className="overlap-group4-item">
                  <div className="flex-row">
                    <div className="title-3 plusjakartasans-medium-eerie-black-13px">
                      Twitter
                    </div>
                    <div className="digits-3 plusjakartasans-medium-eerie-black-13px">
                      21,893
                    </div>
                  </div>
                  <div className="overlap-group">
                    {/* <img
                      className="elements-line-gorizontal-3"
                      src="https://anima-uploads.s3.amazonaws.com/projects/630e6c3ef11c17b54f51d1b7/releases/6316430df11c17b54f51f346/img/elements---line---gorizontal-1@2x.svg"
                    /> */}
                  </div>
                </div>
              </div>
              <div className="overlap-group6">
                <div className="recent-customers plusjakartasans-bold-eerie-black-16px">
                  Recent Customers
                </div>
                <p className="lorem-ipsum-dolor-sit-ametis jakartasans-normal-sonic-silver-13px">
                  Lorem ipsum dolor sit ametis.
                </p>
                <div className="customer-1">
                  <div className="customer-name">
                    <div className="name-31 plusjakartasans-bold-eerie-black-13px">
                      Jenny Wilson
                    </div>
                    <div className="examplecom jakartasans-normal-sonic-silver-13px">
                      w.lawson@example.com
                    </div>
                  </div>
                  <div className="customer-details-1">
                    <div className="price-6 plusjakartasans-medium-eerie-black-13px">
                      $11,234
                    </div>
                    <div className="name-2 jakartasans-normal-sonic-silver-13px">
                      Austin
                    </div>
                  </div>
                </div>
                <div className="x3-3">
                  <div className="title-1-10">
                    <div className="name-3 plusjakartasans-bold-eerie-black-13px">
                      Devon Lane
                    </div>
                    <div className="examplecom jakartasans-normal-sonic-silver-13px">
                      dat.roberts@example.com
                    </div>
                  </div>
                  <div className="title-2-1">
                    <div className="price-7 plusjakartasans-medium-eerie-black-13px">
                      $11,159
                    </div>
                    <div className="place-10 jakartasans-normal-sonic-silver-13px">
                      New York
                    </div>
                  </div>
                </div>
                <div className="overlap-group6-item">
                  <div className="title-1-20">
                    <div className="name-4 plusjakartasans-bold-eerie-black-13px">
                      Jane Cooper
                    </div>
                    <div className="examplecom jakartasans-normal-sonic-silver-13px">
                      jgraham@example.com
                    </div>
                  </div>
                  <div className="title-2-2">
                    <div className="price-8 plusjakartasans-medium-eerie-black-13px">
                      $10,483
                    </div>
                    <div className="place-20 jakartasans-normal-sonic-silver-13px">
                      Toledo
                    </div>
                  </div>
                </div>
                <div className="overlap-group6-item">
                  <div className="title-1-30">
                    <div className="name-5 plusjakartasans-bold-eerie-black-13px">
                      Dianne Russell
                    </div>
                    <div className="examplecom jakartasans-normal-sonic-silver-13px">
                      curtis.d@example.com
                    </div>
                  </div>
                  <div className="title-2-3">
                    <div className="price-9 plusjakartasans-medium-eerie-black-13px">
                      $9,084
                    </div>
                    <div className="place-30 jakartasans-normal-sonic-silver-13px">
                      Naperville
                    </div>
                  </div>
                </div>
                <div className="see-customers-btn">
                  <div className="see-all-customers">SEE ALL CUSTOMERS</div>
                  <img
                    className="icon-outlinecheveron-right-1"
                    src="https://anima-uploads.s3.amazonaws.com/projects/630e6c3ef11c17b54f51d1b7/releases/6316430df11c17b54f51f346/img/icon-outline-cheveron-right-1@2x.svg"
                  />
                </div>
              </div>
            </div>
          </div>
          <div className="dz-stats">
            <div className="dz-stats-card-1">
              <div className="card-1">
                <div className="todays-sale plusjakartasans-medium-sonic-silver-11px">
                  TODAY’S SALE
                </div>
                <div className="bottom">
                  <div className="price">
                    <div className="price-10 valign-text-bottom plusjakartasans-bold-eerie-black-21px">
                      $12,426
                    </div>
                  </div>
                  <div className="growth">
                    <div className="x36 plusjakartasans-medium-malachite-13px">
                      + 6%
                    </div>
                    <img
                      className="icon-outlinearrow"
                      src="https://anima-uploads.s3.amazonaws.com/projects/630e6c3ef11c17b54f51d1b7/releases/6314fda93cda19c4344df2f6/img/icon-outline-arrow-up@2x.svg"
                    />
                  </div>
                </div>
              </div>
            </div>
            <div className="dz-stats-card">
              <div className="card-1">
                <div className="todays-sale plusjakartasans-medium-sonic-silver-11px">
                  TOTAL SALES
                </div>
                <div className="bottom">
                  <div className="price">
                    <div className="x12426 valign-text-bottom plusjakartasans-bold-eerie-black-21px">
                      $2,38,485
                    </div>
                  </div>
                  <div className="growth">
                    <div className="x36-10">- 14%</div>
                    <img
                      className="icon-outlinearrow"
                      src="https://anima-uploads.s3.amazonaws.com/projects/630e6c3ef11c17b54f51d1b7/releases/6314fda93cda19c4344df2f6/img/icon-outline-arrow-down@2x.svg"
                    />
                  </div>
                </div>
              </div>
            </div>
            <div className="dz-stats-card">
              <div className="card-1">
                <div className="todays-sale plusjakartasans-medium-sonic-silver-11px">
                  TOTAL ORDERS
                </div>
                <div className="bottom">
                  <div className="price">
                    <div className="x12426-1 valign-text-bottom plusjakartasans-bold-eerie-black-21px">
                      84,382
                    </div>
                  </div>
                  <div className="growth">
                    <div className="x36-2 plusjakartasans-medium-malachite-13px">
                      + 12%
                    </div>
                    <img
                      className="icon-outlinearrow"
                      src="https://anima-uploads.s3.amazonaws.com/projects/630e6c3ef11c17b54f51d1b7/releases/6314fda93cda19c4344df2f6/img/icon-outline-arrow-up@2x.svg"
                    />
                  </div>
                </div>
              </div>
            </div>
            <div className="dz-stats-card">
              <div className="card-1">
                <div className="todays-sale plusjakartasans-medium-sonic-silver-11px">
                  TOTAL CUSTOMERS
                </div>
                <div className="bottom">
                  <div className="price">
                    <div className="x12426-2 valign-text-bottom plusjakartasans-bold-eerie-black-21px">
                      33,493
                    </div>
                  </div>
                  <div className="growth">
                    <div className="x36-3 plusjakartasans-medium-malachite-13px">
                      + 26%
                    </div>
                    <img
                      className="icon-outlinearrow"
                      src="https://anima-uploads.s3.amazonaws.com/projects/630e6c3ef11c17b54f51d1b7/releases/6314fda93cda19c4344df2f6/img/icon-outline-arrow-up@2x.svg"
                    />
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}

export default DropzoneDashboard;
