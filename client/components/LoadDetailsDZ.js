...
import { thunk_fetchSingleLoad, thunk_updateLoadStatus } from "../store/loads";
                ...
                this.props.jumpRecords.map((user, index) => {
                  return (
                    <tr key={index}>
                      ...
                      >
                        {this.props.singleLoad.date.slice(0, 9) ==
                        todaysDate ? (
                          <button
...
const mapDispatch = (dispatch) => {
  return {
    editLoad: (userId, loadId) => dispatch(thunk_updateLoad(userId, loadId)),
    editLoadStatus: (status, loadId) =>
      dispatch(thunk_updateLoadStatus(status, loadId)),
